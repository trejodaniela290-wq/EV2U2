...
# Búsqueda binaria sobre una lista de clientes ordenada por nombre
def busquedaCliente(clientes_ordenados, nombre_cliente):
    """
    La lista de clientes se ordena alfabeticamente en la funcion buscar_cliente
    Asi podemos usar un while para buscar entre los nombres ordenados
    Retorna el índice del cliente si se encuentra, -1 si no existe.
    """
    nombre_cliente = nombre_cliente.lower()
    izquierda, derecha = 0, len(clientes_ordenados) - 1
...
class SistemaBanco:
    ...
    def agregarCliente(self, nombre, numero_cuenta):
        cliente = Cliente(nombre, numero_cuenta)
        self.cola.agregar(cliente)
        self.historial.registrar("agregar", cliente)
        print(f"  Cliente agregado: {cliente}")
    ...
    def deshacerMovimiento(self):
        ...
        if tipo == "agregar":
            ...
            if cliente in clientes_actuales:
                ...
                for c in clientes_actuales:
                    self.cola.agregar(c)
                print(f" Se eliminó a {cliente.nombre} de la fila.")
            else:
                print(f"  El cliente {cliente.nombre} ya fue atendido")
            ...
            for c in self.cola.lista_clientes():
                nueva_cola.agregar(c)
            self.cola = nueva_cola
            print(f"{cliente.nombre} regresó al frente de la fila.")
    ...
    def buscarCliente(self, nombre):
        ...
        # Ordenar copia por nombre para aplicar búsqueda binaria
        clientes_ordenados = sorted(clientes, key=lambda c: c.nombre.lower())
        indice = busquedaCliente(clientes_ordenados, nombre)
...
def menu():
    ...
    while True:
        ...
        if opcion == "1":
            ...
            cuenta = input("  Número de cuenta: ").strip()
            if nombre and cuenta:
                sistema.agregarCliente(nombre, cuenta)
            else:
                print("  Nombre y número de cuenta son obligatorios.")
            ...
            sistema.deshacerMovimiento()

        elif opcion == "4":
            nombre = input("  Nombre a buscar: ").strip()
            sistema.buscarCliente(nombre)
            ...
            break #CASO BASE: Que el turno del banco se acabe y todos los clientes ya fueron atendidos
...
if __name__ == "__main__":
>>>>>>> fb972e6 (Agregar EV2U2_filaDeBanco.py al repositorio)
    menu()